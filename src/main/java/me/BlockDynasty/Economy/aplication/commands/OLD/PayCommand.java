...
/*
    ...
    public PayCommand(BlockDynastyEconomy plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String s13542415, String[] args) {
        ...
        SchedulerUtils.runAsync(() -> {
            ...
            if (currencyManager.getDefaultCurrency() == null) {
                        ...
                        if (!target.getUuid().equals(account.getUuid())) {
                            if (target.canReceiveCurrency()) {
                                if (account.hasEnough(currency, amount)) {
                                    ...
                                    accountManager.modifyBalance(account,currency, accBal, true);
                                    accountManager.modifyBalance(target,currency, tarBal, true);
                                    plugin.getEconomyLogger().log("[PAYMENT] " + account.getDisplayName() + " (New bal: " + currency.format(accBal) + ") -> paid " + target.getDisplayName() + " (New bal: " + currency.format(tarBal) + ") - An amount of " + currency.format(amount));
