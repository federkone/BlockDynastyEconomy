...
package net.blockdynasty.economy.sponge.common.adapters.GUI.listener;
    ...
    public boolean handle(Cause cause, Container container, Slot slot, int slotIndex, ClickType<?> clickType) {
        cause.first(ServerPlayer.class).ifPresent(player -> {
            if (ClickTypes.CLICK_LEFT.get().equals(clickType)){
                GUISystem.handleClick(EntityPlayerAdapter.of(player), net.blockdynasty.economy.gui.gui.components.ClickType.LEFT, slotIndex);
            } else if (ClickTypes.CLICK_RIGHT.get().equals(clickType)) {
                GUISystem.handleClick(EntityPlayerAdapter.of(player), net.blockdynasty.economy.gui.gui.components.ClickType.RIGHT, slotIndex);
            }
        });
