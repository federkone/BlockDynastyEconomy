...
public abstract class PaginatedGUI<T> extends AbstractGUI {
    ...
    public PaginatedGUI(String title, int rows, ServerPlayer player, IGUI parent, int itemsPerPage) {
    ...
    protected ItemStack createEmptyMessage() {
        return createItem(ItemTypes.BARRIER.get(), "§cNo Elements", "§7There are no items to display");
    }

    protected ItemStack createPreviousButton() {
        return createItem(ItemTypes.ARROW.get(), "§aPrevious page", "§7Click to see previous items");
    }

    protected ItemStack createNextButton() {
        return createItem(ItemTypes.ARROW.get(), "§aNext Page", "§7Click to see more items");
    }

    protected ItemStack createBackButton() {
        return createItem(ItemTypes.BARRIER.get(), "§cBack", "§7Click to go back");
    }
