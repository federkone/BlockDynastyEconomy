    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        Scheduler.runAsync(ContextualTask.build(() -> {
            try {
                ...
                sender.sendMessage(Message.getPrefix() + "§7Toggled Decimal Support for §f" + currencyName);
            } catch (CurrencyNotFoundException e) {
                sender.sendMessage(Message.getUnknownCurrency());
            } catch (TransactionException e) {
                sender.sendMessage(Message.getPrefix() + "§cError toggling decimals for currency §f" + currencyName);
            }
        }));
        return false;
    }
